        ...
        for y in y_min..(y_max + 1) {
            ...
            if u_a >= 0.0 && u_b >= 0.0 && u_c >= 0.0 {
                ...
                if zbuffer[x as usize][y as usize] < z {
                    ...
                    let tex_coord = u_a * a.tex_coord
                        + (u_b * b.tex_coord).to_vec()
                        + (u_c * c.tex_coord).to_vec();
...
fn ortho(v: Point3<f32>) -> Point3<f32> {
    Point3::new(
...
fn perspective_mat(near: f32, far: f32) -> Matrix4<f32> {
    Matrix4::new(
        near,
        0.0,
        0.0,
...
fn model_mat(translate: Vector3<f32>, scale: Vector3<f32>) -> Matrix4<f32> {
    Matrix4::new(
        scale[0],
        0.0,
        0.0,
...
fn screen_to_raster(fov: f32, near: f32, screen_coord: Point3<f32>) -> Option<Point3<f32>> {
    ...
    if screen_coord.x.abs() > half_width || screen_coord.y.abs() > half_height {
        // some part of the triangle is outside the frustum
        // discard it, though this is not what happens in real life
        None
    } else {
        ...
        Some(Point3::new(x, y, screen_coord.z))
    }
}
...
fn draw_african_head(image: &mut MyImage) {
    ...
    for (i, abc) in model.faces.iter().enumerate() {
        let a = model.verts[abc[0]];
        let b = model.verts[abc[1]];
        let c = model.verts[abc[2]];

        let screen_a = Point3::from_homogeneous(mvp * Vector4::new(a.x, a.y, a.z, 1.0));
        let screen_b = Point3::from_homogeneous(mvp * Vector4::new(b.x, b.y, b.z, 1.0));
        let screen_c = Point3::from_homogeneous(mvp * Vector4::new(c.x, c.y, c.z, 1.0));

        if let (Some(pos_a), Some(pos_b), Some(pos_c)) = (
            screen_to_raster(FOV, NEAR, screen_a),
            screen_to_raster(FOV, NEAR, screen_b),
            screen_to_raster(FOV, NEAR, screen_c),
        ) {
            let tex_a = Point2::new(
                model.texcoords[2 * model.texcoords_indices[i][0]],
                model.texcoords[2 * model.texcoords_indices[i][0] + 1],
            );
            let tex_b = Point2::new(
                model.texcoords[2 * model.texcoords_indices[i][1]],
                model.texcoords[2 * model.texcoords_indices[i][1] + 1],
            );
            let tex_c = Point2::new(
                model.texcoords[2 * model.texcoords_indices[i][2]],
                model.texcoords[2 * model.texcoords_indices[i][2] + 1],
            );
            ...
            let v0 = Vertex::new(pos_a, tex_a, normal_a);
            let v1 = Vertex::new(pos_b, tex_b, normal_b);
            let v2 = Vertex::new(pos_c, tex_c, normal_c);
